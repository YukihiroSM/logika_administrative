...
class TeacherFeedbackForm(forms.Form):
    lesson_mark = forms.IntegerField(
        min_value=1, max_value=10, required=True, label="Оцінка уроку"
    )
    mistakes = forms.CharField(max_length=256, required=True, label="Помилки")
    problems = forms.MultipleChoiceField(
        choices=PROBLEMS_CHOICES, required=True, label="Проблеми"
    )
    additional_problems = forms.CharField(
        max_length=1024, label="Додаткові проблеми", required=False
    )
    technical_problems = forms.CharField(
        max_length=1024, required=False, label="Технічні проблеми"
    )
    km_work_mark = forms.IntegerField(
        min_value=1, max_value=10, required=True, label="Оцінка роботи КМ"
    )
    km_work_comment = forms.CharField(
        max_length=1024, required=False, label="Коментарі до роботи КМ"
    )
    tutor_work_mark = forms.IntegerField(
        min_value=1, max_value=10, required=True, label="Оцінка роботи тьютора"
    )
    tutor_work_comment = forms.CharField(
        max_length=1024, required=False, label="Коментарі до роботи тьютора"
    )
