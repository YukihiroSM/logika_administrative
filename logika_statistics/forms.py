...
class AddStudentId(forms.Form):
    lms_id = forms.CharField(label="ID учня", max_length=100, required=True)
...
REGIONALS = [
    ("Кравченко Олеся", "Кравченко Олеся"),
    ("Ласота Юрій", "Ласота Юрій"),
...
class AssignIssueForm(forms.Form):
    territorial_manager = forms.ChoiceField(
        label="Територіальний менеджер", choices=TERRITORIAL_MANAGERS, required=True
    )
...
class AddLocationForm(forms.Form):
    location = forms.ChoiceField(label="Локація", choices=LOCATIONS, required=True)


class AssignTutorIssueForm(forms.Form):
    tutor = forms.ChoiceField(label="Т`ютор", choices=TUTORS, required=True)


class AddTeacherForm(forms.Form):
    teacher = forms.CharField(label="Викладач", max_length=256, required=True)
