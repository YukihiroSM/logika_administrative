...
from logika_statistics.models import StudentReport, GlobalGroup, Issue, Location
    ...
    def handle(self, *args, **options):
        ...
        for payment in payments_data:
            ...
            if payment["Оплата"] < 500:
                print(
                    f"СЛИШКОМ МАЛЕННЬКАЯ ОПЛАТА!!!! {str(payment['КлиентID_БО'])}"
                    + " "
                    + str(datetime.now())
                )
                issue = Issue(
            ...
            existing_report = StudentReport.objects.filter(
                student_lms_id=student_id, business=business, attended_mc=1
            ).first()
            if existing_report:
                client_manager = None
                ...
                print(
                    "FOUND EXISTING REPORT FOR" + str(student_id) + str(datetime.now())
                )
                report = StudentReport(
                    ...
                    if client_manager
                    else existing_report.client_manager,
                    territorial_manager=territorial_manager
                    if territorial_manager
                    else existing_report.territorial_manager,
                    regional_manager=regional_manager
                    if regional_manager
                    else existing_report.regional_manager,
            ...
            if resp.status_code == 200:
                ...
                print(
                    f"Creating report for {str(student_id)}" + " " + str(datetime.now())
                )
                if business == "programming":
                    tutor = (
                        last_group.location.tutor
                        if last_group and last_group.location
                        else None
                    )
                elif business == "english":
                    tutor = (
                        last_group.location.tutor_english
                        if last_group and last_group.location
                        else None
                    )
                else:
                ...
                if business == "programming":
                    cm = (
                        last_group.location.client_manager
                        if last_group and last_group.location
                        else None
                    )
                elif business == "english":
                    cm = (
                        last_group.location.client_manager_english
                        if last_group and last_group.location
                        else None
                    )
                else:
