...
class Command(BaseCommand):
    ...
    def process_one_group(self, group_id):
        ...
        for student in data_dict:
            attendance[student["student_id"]] = (
                1 if student["attendance"][0]["status"] == "present" else 0
            )
        ...
        for student in students["items"]:
            ...
            try:
                ...
                existing_issue = Issue.objects.filter(
                    ...
                    report_start=issue.report_start,
                ).first()
                if existing_issue:
    ...
    def is_duplicates(self, student_id):
        ...
        if len(phone) > 1:
            try:
                ...
                return False
            normal_number = f"{parsed_phone.country_code}{parsed_phone.national_number}"
            res = self.global_students_df[
                self.global_students_df["Тел. родителя"] == normal_number
            ]
            if len(res) > 1:
    ...
    def handle(self, *args, **options):
        ...
        output_file_path = Path(
            BASE_DIR, "lms_reports", month, f"{start_date}_{end_date}", "schedule.csv"
        )
        ...
        for index, row in mk_df.iterrows():
            ...
            if row["group_location"]:
                if business == "programming":
                    ...
                    if location:
                        ...
                        print(
                            f"LOCATION NOT IN LIST {row['group_location']}"
                            + " "
                            + str(datetime.now())
                        )
                        issue = Issue(
            ...
            for student in students:
                ...
                report = StudentReport(
                    ...
                    student_mk_group_id=GlobalGroup.objects.filter(
                        lms_id=row["group_id"]
                    ).first(),
