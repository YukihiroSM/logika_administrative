    ...
    def handle(self, *args, **options):
        ...
        for idx, row in df.iterrows():
            ...
            if row["КМ программирование"] == "":
                row[
                    "КМ программирование"
                ] = f"Невідомий КМ {row['Название локации в ЛМС']}"
            if row["КМ английский"] == "":
                row[
                    "КМ английский"
                ] = f"Невідомий КМ англ {row['Название локации в ЛМС']}"
            if row["ТЬЮТОР программирования"] == "":
                row[
                    "ТЬЮТОР программирования"
                ] = f"Невідомий т'ютор {row['Название локации в ЛМС']}"
            if row["ТЬЮТОР англ"] == "":
                row[
                    "ТЬЮТОР англ"
                ] = f"Невідомий т'ютор англ {row['Название локации в ЛМС']}"
            location = Location.objects.filter(
                lms_location_name=row["Название локации в ЛМС"].strip()
            ).first()
            if location:
                print(
                    f"Updated location {row['Название локации в ЛМС']}",
                ...
                print(
                    f"Created location {row['Название локации в ЛМС']}",
                    str(datetime.datetime.now()),
                )
                location = Location(
                    lms_location_name=row["Название локации в ЛМС"].strip(),
